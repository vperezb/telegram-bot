...
def telegram_bot(request):
    ...
    if request.method == "POST":
        ...
        if chat_id in name_users:
            nom = name_users[chat_id]['name']
        else:
            nom = chat_id
        ...
        if response['result'] == 'success':
            #bot.sendMessage(chat_id=chat_id, text='Hey {} la teva comanda s''ha registrat a la fila {}'.format(name_users[chat_id]['name'], response['row']))    
            for key in name_users:
...
def save_to_googlesheets(payload):
    try:
        URL = 'https://script.google.com/macros/s/ID-SCRIPT/exec'
